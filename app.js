...
let cateChoosing = "electronics";


// let categoryChoosing = ''

// function changeCate(category) {
//   cateChoosing = category;
//   console.log(cateChoosing);
// }
...
const options = {
  method: "GET",
  headers: {
    "X-RapidAPI-Key": "6cf2ad7622msh5c5dd0c3cd2f693p108991jsne7365c7549f7",
...
(async () => {
  ...
  const data = [
    ...
    },
    {
        ...
        "description": "Rethink the basketball trainer with the Rebound Lay-Up. This versatile sneaker features a mesh upper for superior breathability and a SoftFoam+ sockliner to give you a supportive fit. Paired with the iconic PUMA Formstrip, you will  have classic style with all of the innovation of today.",
        "quantity": 1,
        "rating": {
            "rate": 4.8,
            "count": 400
    ...
    },
    {
        ...
        "description": "Utilitarian, but make it edgy: these AJ1s give you functional use without compromising that style you know and love. Tough canvas, stash pockets and webbing pulls give these classic kicks a hit of rugged versatility.",
        "quantity": 1,
        "rating": {
            "rate": 1.7,
            "count": 160
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    price.innerHTML = "<b>Price: </b>" + data[i].price + "$";
    ...
    button.setAttribute(
      "onclick",
      `addItemIntoCard("${data[i].name}",'${data[i].price} $')`
    );
...
function submitInfo() {
  alert('Chúng tôi đã nhận thông tin của xin chân thành cảm ơn');
  return;
}
...
function displayCartItems() {
  ...
  items.forEach(function (item) {
    ...
    li.textContent = item.item + " Price " + item.price;
    cartItemsElement.appendChild(li);
  });
}

const addItemIntoCard = (item, price) => {
  items.push({
    item: item,
    price: price,
  });
  alert(`Đã thêm ${item} vào giỏ hàng`);
  displayCartItems();
};
